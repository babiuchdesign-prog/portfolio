...
> = ({ imageItems, title, description }) => {
  ...
  return (
    <section
      ref={targetRef}
      className="relative w-full overflow-hidden bg-transparent pt-4 pb-16 sm:pt-8 sm:pb-24"
    >
      <motion.div
        style={{ opacity, y }}
      ...
      <div className="relative mt-12 w-full mx-auto px-4 md:px-8 max-w-[1400px]">
        <motion.div
          ref={gridRef}
          className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 auto-rows-[15rem] md:auto-rows-[18rem] gap-4"
        ...
        >
          {imageItems.map((item) => (
            <motion.div
              ...
              className={cn(
                "group relative flex h-full min-h-[15rem] w-full cursor-pointer items-end overflow-hidden rounded-xl border border-white/10 bg-white/5 p-4 shadow-sm transition-shadow duration-300 ease-in-out hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white/50 focus-visible:ring-offset-2 focus-visible:ring-offset-black",
                item.span,
              )}
