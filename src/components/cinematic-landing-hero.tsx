...
const INJECTED_STYLES = `
  ...
  .film-grain {
      ...
      pointer-events: none; z-index: 50; opacity: 0; mix-blend-mode: overlay;
      background: none;
  }
  ...
  .text-3d-matte {
      color: #ffffff;
      text-shadow: 
          0 10px 30px color-mix(in srgb, #ffffff 20%, transparent), 
          0 2px 4px color-mix(in srgb, #ffffff 10%, transparent);
  }

  .text-silver-matte {
      background: linear-gradient(180deg, #ffffff 0%, color-mix(in srgb, #ffffff 40%, transparent) 100%);
      -webkit-background-clip: text;
      -webkit-text-fill-color: transparent;
      background-clip: text;
      padding: 0.15em 0; /* Prevents descender clipping */
      transform: translateZ(0); /* Hardware acceleration to prevent WebKit clipping bug */
      filter: 
          drop-shadow(0px 10px 20px color-mix(in srgb, #ffffff 15%, transparent)) 
          drop-shadow(0px 2px 4px color-mix(in srgb, #ffffff 10%, transparent));
  }
...
export function CinematicHero({ 
  ...
  tagline1 = "Projekty",
  tagline2 = "aplikacji",
  cardHeading = "Opracowanie aplikacji mobilnej",
  cardDescription = <><span className="text-white font-semibold">Kompleksowe podejście</span> – od pierwszej wizji i makiet w Figmie, przez zaawansowane wdrożenie z wykorzystaniem Antigravity, Fluttera i VS Code, aż po finalną publikację gotowego projektu w sklepie Google Play.</>,
  metricValue = 365,
  metricLabel = "Days Sober",
  ctaHeading = "Przetestuj Florium.",
  ctaDescription = "Sprawdź, jak wsparcie sztucznej inteligencji odmienia codzienną pielęgnację roślin i stwórz swój własny, inteligentny zielnik.",
  className, 
  ...props 
}: CinematicHeroProps) {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(".text-days", { autoAlpha: 1, clipPath: "inset(-20% 100% -20% -10%)" });
      ...
      introTl
        ...
        .to(".text-days", { duration: 1.4, clipPath: "inset(-20% -10% -20% -10%)", ease: "power4.inOut" }, "-=1.0");
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative w-screen h-screen overflow-hidden flex items-center justify-center bg-transparent text-white font-sans antialiased", className)}
      style={{ perspective: "1500px" }}
      {...props}
    >
      <style dangerouslySetInnerHTML={{ __html: INJECTED_STYLES }} />
      <div className="film-grain opacity-0" aria-hidden="true" />
      <div className="bg-grid-theme absolute inset-0 z-0 pointer-events-none opacity-0" aria-hidden="true" />

      {/* BACKGROUND LAYER: Hero Texts */}
      <div className="hero-text-wrapper absolute z-10 flex flex-col items-center justify-center text-center w-screen px-4 will-change-transform transform-style-3d">
        <h1 className="flex items-center justify-center flex-wrap gap-x-4 text-5xl md:text-7xl lg:text-[6rem] font-bold tracking-tight mb-2">
          <span className="text-track gsap-reveal text-3d-matte">
            {tagline1}
          </span>
          <span className="text-days gsap-reveal text-silver-matte font-extrabold tracking-tighter">
            {tagline2}
          </span>
        </h1>
      </div>
        ...
        <div className="flex justify-center w-full mt-2">
          <a href="https://play.google.com/store/apps/details?id=com.babiuchdesigndev.plantdex&hl=pl" target="_blank" rel="noopener noreferrer" aria-label="Get it on Google Play" className="btn-modern-dark flex items-center justify-center gap-3 px-8 py-4 rounded-[1.25rem] group focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-background">
            ...
            <div className="text-left">
              <div className="text-[10px] font-bold tracking-wider text-neutral-400 uppercase mb-[-2px]">Ściągnij z</div>
            ...
            <div className="mockup-scroll-wrapper order-2 lg:order-2 relative w-full h-[380px] lg:h-[600px] flex items-center justify-center z-10" style={{ perspective: "1000px" }}>
              ...
              <div className="relative w-full h-full flex items-center justify-center transform scale-[0.65] md:scale-85 lg:scale-100">
                ...
                <div className="floating-badge absolute flex top-6 lg:top-12 left-[-15px] lg:left-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Skaner roślin i chorób</p>
                    <p className="text-green-200/50 text-[10px] lg:text-xs font-medium">Identyfikacja AI</p>
                  </div>
                </div>
                ...
                <div className="floating-badge absolute flex bottom-12 lg:bottom-20 right-[-15px] lg:right-[-80px] floating-ui-badge rounded-xl lg:rounded-2xl p-3 lg:p-4 items-center gap-3 lg:gap-4 z-30">
                  ...
                  <div>
                    <p className="text-white text-xs lg:text-sm font-bold tracking-tight">Odkrywaj nowe gatunki</p>
                    <p className="text-emerald-200/50 text-[10px] lg:text-xs font-medium">Dodawaj do kolekcji</p>
                  </div>
                </div>
